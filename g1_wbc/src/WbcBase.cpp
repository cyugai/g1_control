...
{
  qMeasured_.head<3>() = rbdStateMeasured.segment<3>(3);  // xyz linear pos
  qMeasured_.segment<3>(3) = rbdStateMeasured.head<3>();  //  euler angles
...
{
  static vector_t contact_filted = inputDesired.head(contact_force_size_);
  ...
  contact_filted = contact_filted * contactForceFilter_ + inputDesired.head(a.rows()) * (1 - contactForceFilter_);
  b = contact_filted;
...
void WbcBase::loadTasksSetting(const std::string& taskFile, bool verbose)
{
  ...
  loadData::loadPtreeValue(pt, contactForceFilter_, "contactForceFilter", verbose);
}
